...
class ForgeryDocument(Superclass):
	mode = ID.DRAW_MODE
	modes = None
	
	currentMode = property(fget = lambda self: self.modes[self.mode])
	undoer = property(fget = lambda self: self.data.undoManager)
	...
	if usePyObjC:
		...
		statusBar = objc.ivar('statusBar')
		window = objc.ivar('myWindow')
	...
	def importShapes_(self, sender):
		...
		result = panel.runModalForDirectory_file_types_(NSHomeDirectory(), None, ['shpA', NSFileTypeForHFSTypeCode('shpA')])
		if result == NSOKButton:
			self.importShapes(panel.filenames()[0])
		#self.importShapes("/Applications/Marathon Infinity.a1bndl/Shapes.shpA")
